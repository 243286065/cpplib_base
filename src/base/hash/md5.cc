...
bool FastFileMD5(const std::string& file_path, std::string& result) {
  ...
  while (!ifs.eof())
  {
    ...
    md5.update((const byte*)buf, (size_t)ifs.gcount());
  }
  ifs.close();
...
MD5::MD5(const std::string& message) {
  ...
  update((const byte*)message.c_str(), message.length());
}
